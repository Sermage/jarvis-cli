...
class QueryRewriter(Protocol):
    """Переформулировщик запроса для поиска (первый этап улучшенного RAG).

    Раскрывает аббревиатуры, добавляет термины/синонимы, убирает воду —
    чтобы эмбеддинг запроса лучше «попадал» в релевантные чанки. Реализация
    (`infra/query_rewriter.py`) использует LLM; при любой ошибке обязана
    вернуть исходный запрос, а не падать.
    """

    def rewrite(self, query: str) -> str:
        """Вернуть переформулированный запрос (или исходный, если не вышло)."""
        ...


class Reranker(Protocol):
    """Переупорядочивание найденных чанков (второй этап, после поиска).

    Косинусный поиск (bi-encoder) грубоват: нужный чанк может оказаться ниже
    по рангу. Reranker пересматривает порядок кандидатов более точным
    сигналом (лексическим или cross-encoder на LLM). Не обязан сохранять
    длину списка, но обычно возвращает те же чанки в новом порядке.
    """

    def rerank(self, query: str, chunks: list[RetrievedChunk]) -> list[RetrievedChunk]:
        """Вернуть чанки, переупорядоченные по релевантности запросу."""
        ...
