...
Терминальный чат с LLM — интерактивный CLI с трёхуровневой моделью памяти.
По умолчанию используется [DeepSeek](https://platform.deepseek.com), но
можно переключиться на [GigaChat](https://developers.sber.ru/studio) — провайдер
выбирается через переменную окружения `LLM_PROVIDER` или командой `/provider`
прямо в REPL.
...
# опционально, рекомендуется на macOS:
pip3 install gnureadline
```

`gnureadline` подменяет встроенный `readline` (на macOS Python это libedit)
на полноценный GNU readline 8+. Без него многострочный paste при отправке
в чат показывает видимые `^[[200~`/`^[[201~`-маркеры в строке ввода —
функционально работает корректно, но визуально шумно. С `gnureadline`
маркеры подавляются терминалом, и paste выглядит как в bash/zsh.

**3. Добавить ключ авторизации**

```bash
cp .env.example .env
# открыть .env и вписать ключи нужных провайдеров:
#   DEEPSEEK_API_KEY=...   (по умолчанию)
#   GIGACHAT_AUTH_KEY=...  (опционально)
# опционально: LLM_PROVIDER=deepseek|gigachat
```
...
| Команда | Описание |
|---|---|
| `/provider` | Переключить LLM-провайдера (deepseek / gigachat) |
